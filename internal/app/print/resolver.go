...
func (r *resolver) Resolve(format dp.SourceFormat, profile dp.PrinterProfile) (dp.Pipeline, error) {
	if len(profile.NativeFormats) == 0 {
		return dp.Pipeline{}, fmt.Errorf("print: printer %q has no native formats", profile.PrinterID)
	}
	...
	return dp.Pipeline{}, fmt.Errorf("print: no pipeline for source %q -> device %v", format, profile.NativeFormats)
}
